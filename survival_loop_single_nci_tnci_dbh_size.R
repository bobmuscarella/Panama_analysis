...
###  SURVIVAL ANALYSIS
###  SINGLE-PLOT
###  NCI, tNCI, DBH WITH DIFF SIZE CLASSES
###  RESULTS PARTIAL in "K:/Bob/Panama/RESULTS/_12.17.15/survival/nci_tnci_dbh_size"
#######################################
library(jagsUI)
  ...
  for(p in 1:3){
    ...
    for(size in 1) {
      ...
      data = list (
        ...
        tnci = as.numeric(dps[,paste('log.tnci.', trait, '.z', sep='')]),
    ...
    , fill=TRUE)
      ...
      setwd("K:/Bob/Panama/RESULTS/_12.17.15/survival/nci_tnci_dbh_sizes") 
