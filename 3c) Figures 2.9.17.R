setwd("/Users/Bob/Projects/Postdoc/Panama/RESULTS/_2.16.17")
#library(jagsUI)
library(rjags)
library(runjags)
library(RColorBrewer)

#### THIS OLD LOOP WORKED FOR JAGSUI OBJECTS...
# res <- data.frame()
# for(j in 1:2){
...
sout <- read.csv("/Users/Bob/Projects/Postdoc/Panama/RESULTS/_2.19.17_stan/survival/output.csv", row.names=1)
sout$param <- ifelse(sout$param=='mu1','mu.beta[1]',
                     ifelse(sout$param=='mu2','mu.beta[2]',
                     ifelse(sout$param=='mu3','mu.beta[3]',
                     ifelse(sout$param=='t1','beta.wd[1]',
                     ifelse(sout$param=='t2','beta.lma[1]',
                     ifelse(sout$param=='t3','beta.wd[2]',
                     ifelse(sout$param=='t4','beta.lma[2]',NA)))))))

sout[,5:9] <- round(sout[,5:9], 3)
...
col2 <- addalpha(brewer.pal(11, 'BrBG')[c(3,9,11)], 0.25)
...
plot_average_rates <- function(d1, d2, mtxt=NULL){
  ...
  colreps2 <- rep(rep(col2, each=2), times=2)
  # bg <- ifelse(sign(d1$q2.5)==sign(d1$q97.5), colreps, ifelse(sign(d1$q5)==sign(d1$q95), colreps2, 'white'))
  ...
  # segments(xs, d1$q5, xs, d1$q95, col=colreps, lwd=4, lend=1)
  ...
  # bg <- ifelse(sign(d2$q2.5)==sign(d2$q97.5), colreps, ifelse(sign(d2$q5)==sign(d2$q95), colreps2, 'white'))
  bg <- ifelse(sign(d2$q2.5)==sign(d2$q97.5), colreps, 'white')
  ...
  # segments(xs, d2$q5, xs, d2$q95, col=colreps, lwd=4, lend=1)
...
plot_nontrait_effects <- function(d1, d2, mtxt=NULL){
  ...
  points(xs, d1$q50, pch=c(21,23), bg=bg, col=colreps, cex=1.5)
  axis(2); box()
  mtext(mtxt, 2, 4, font=2)
  mtext('Standard Effect Size', 2, 2, font=1)
  mtext('(A) Growth', 3, 0.25, font=2, at=0.75, adj=0)
  xs2 <- seq(1.125, 3.125, by=1)
  mtext(rep(c('Dry','Intermediate','Wet')), side=3, line=-1, at=xs2, cex.axis=1.5, col=col, font=2)
  #   mtext(rep(c('Cocoli','BCI','Sherman')), side=3, line=-1, at=xs2, cex.axis=1.5, col=col, font=2)
...
#pdf(file="/Users/Bob/Desktop/avg_rates.pdf")
g <- res[res$m=='growth',]
b <- c('mu.beta[1]')
...
#pdf(file="/Users/Bob/Desktop/trait_effects_on_avg_rates.pdf")
g <- res[res$m=='growth',]
...
pdf(file="/Users/Bob/Projects/Postdoc/Panama/Figures/Fig2_trait_effect_on_NCIsensitivity.pdf")
g <- res[res$m=='growth',]
...
pdf(file="/Users/Bob/Projects/Postdoc/Panama/Figures/Fig1_avg_effect_of_NCI.pdf")
g <- res[res$m=='growth',]
b <- c('mu.beta[2]')
d1 <- g[as.character(g$param) %in% b,]
d1 <- d1[order(rev(d1$param), d1$p, rev(d1$s)),]
...
#pdf(file="/Users/Bob/Desktop/size_effect.pdf")
g <- res[res$m=='growth',]
b <- c('mu.beta[3]')
d1 <- g[as.character(g$param) %in% b,]
d1 <- d1[order(rev(d1$param), d1$p, rev(d1$s)),]
...
pdf(file="/Users/Bob/Projects/Postdoc/Panama/Figures/Fig1_avg_effect_of_NCI_V2.pdf")
#pdf(file="/Users/Bob/Desktop/avg_effect_of_NCI.pdf")
...
par(mfrow=c(2,1), mar=c(0,5,2,0), oma=c(3,6,4,8))
  xs <- c(1,1.25, 2, 2.25, 3, 3.25)
  colreps <- rep(rep(col, each=2), times=2)
  colreps2 <- rep(rep(col2, each=2), times=2)
  # bg <- ifelse(sign(d1$q2.5)==sign(d1$q97.5), colreps, ifelse(sign(d1$q5)==sign(d1$q95), colreps2, 'white'))
  ...
  mtext('(+) Crowding 
        increases 
        performance', 2, las=2, 4, cex=0.75, at=ylim[2]/1.3, font=2)
  mtext('(-) Crowding 
        decreases 
        performance', 2, las=2, 4, cex=0.75, at=ylim[1]/1.3, font=2)
  bg <- ifelse(sign(d2$q2.5)==sign(d2$q97.5), colreps, 'white')
  # bg <- ifelse(sign(d2$q2.5)==sign(d2$q97.5), colreps, ifelse(sign(d2$q5)==sign(d2$q95), colreps2, 'white'))
  xs <- rep(1:(nrow(d2)/2), each=2) + rep(c(0, 0.25), times=(nrow(d2)/2))
  ylim <- c(min(d2$q2.5, na.rm=T) - (1*sd(d2$q50)), max(d2$q97.5, na.rm=T) + (2*sd(d2$q50)))
...
pdf(file="/Users/Bob/Projects/Postdoc/Panama/Figures/Fig2_trait_effect_on_NCIsensitivity_V2.pdf")
#pdf(file="/Users/Bob/Desktop/trait_effect_on_NCIsensitivity.pdf")
...
par(mfrow=c(2,1), mar=c(0,6,2,0), oma=c(3,5,4,1))
...
mtext('(+) Acquisitive
      species more
      sensitive to
      crowding', 2, las=2, 4.25, cex=0.75, at=ylim[2]/1.3, font=2)
mtext('(-) Conservative
      species more
      sensitive to
      crowding', 2, las=2, 4.25, cex=0.75, at=ylim[1]/1.3, font=2)
